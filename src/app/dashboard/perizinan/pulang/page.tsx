...
import { Home, Plus, X, Check, XCircle } from 'lucide-react'
...
export default function PerizinanPulangPage() {
  const [list, setList] = useState<PerizinanPulang[]>([])
  const [santriList, setSantriList] = useState<Santri[]>([])
  const [musyrifList, setMusyrifList] = useState<Musyrif[]>([])
  const [showModal, setShowModal] = useState(false)
  const [loading, setLoading] = useState(true)
  const [form, setForm] = useState({
    santriId: '', musyrifId: '', departDate: '', returnDate: '', reason: '',
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    await fetch('/api/perizinan/pulang', {
      ...
      <div className="toolbar">
        ...
        <div className="toolbar-right">
          <button className="btn btn-primary" onClick={() => {
            setForm({ santriId: '', musyrifId: '', departDate: '', returnDate: '', reason: '' })
            setShowModal(true)
          }}>
        ...
        <div className="card-body" style={{ padding: 0 }}>
          {loading ? (
            ...
            <div className="empty-state">
              ...
              <h3>Belum ada perizinan pulang</h3>
              <p>Buat perizinan pulang untuk santri</p>
            </div>
          ) : (
                ...
                <tbody>
                  {list.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td>{new Date(p.departDate).toLocaleDateString('id-ID')}</td>
                      <td>{new Date(p.returnDate).toLocaleDateString('id-ID')}</td>
                      <td style={{ maxWidth: '150px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.reason}</td>
                      <td>
                        <span className={`badge ${p.parentConfirm ? 'badge-success' : 'badge-warning'}`}>
                          {p.parentConfirm ? 'Konfirmasi' : 'Belum'}
                        </span>
                      </td>
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3 className="modal-title">Buat Perizinan Pulang</h3>
              ...
              <div className="modal-body">
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    <input className="form-input" type="date" value={form.departDate} onChange={(e) => setForm({ ...form, departDate: e.target.value })} required />
                  </div>
                  <div className="form-group">
